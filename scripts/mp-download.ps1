...
function HarvestNativeBinding {
    ...
    try {
        ...
        switch ($loadmethod) {
            'node' {
                # use the node version for the path ( implemented by binding)
...
function CleanReleaseFolder ($module_folder) {
    ...
    Remove-Item "$module_folder/compiled" -Recurse -Force -ErrorAction SilentlyContinue
            ...
            foreach ($platform in $platforms) {
                foreach ($arch in $architectures) {
                    ...
                    if ( $OK ) {
                        ...
                        # no need to show multiple warnings
